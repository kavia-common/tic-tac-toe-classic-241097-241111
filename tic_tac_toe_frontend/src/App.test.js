...
test("supports keyboard navigation within the grid (arrow keys) and move via Enter", () => {
  render(<App />);

  // Initial roving focus should be on square 1
  const square1 = screen.getByRole("button", { name: /square 1 .* empty/i });
  square1.focus();
  expect(square1).toHaveFocus();

  // Move right to square 2
  fireEvent.keyDown(square1, { key: "ArrowRight" });
  const square2 = screen.getByRole("button", { name: /square 2 .* empty/i });
  expect(square2).toHaveFocus();

  // Place mark with Enter
  fireEvent.keyDown(square2, { key: "Enter" });
  expect(screen.getByText(/next player: o/i)).toBeInTheDocument();

  // The square 2 button will now be disabled; ensure some other square is still enabled
  expect(screen.getByRole("button", { name: /square 3 .* empty/i })).toBeEnabled();
});
